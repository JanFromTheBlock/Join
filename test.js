...
function addBoardRender() {
    docID('board').innerHTML = /*html*/` //Suchbereich und Add Task Button werden gerendert
        <div id="board-input">
            <div id="find-task">
    ...
    `
    taskArenaRender(); // muss in die init function
    loadTasks();
}

function taskArenaRender() {
    for (let i = 0; i < taskTitles.length; i++) {  //die 4 Task-areas werden gerendert (To dos, in Progress, ...)
            const taskTitle = taskTitles[i];  //Diese Variable bestimmt, wo das gedarggte Objekt hinverschoben werden soll
            const taskName = taskNames[i];    //Diese variable weißt den task-areas den dazugehörigen Titel zu
            let indexFinal = i + 1             // index muss um einen erhöht werden, da in diesem Fall bei den tasks bei 1 angefangen wurde zu zählen und nicht wie üblich bei 0
            docID('task-area').innerHTML += /*html*/` 
                <div ondrop="moveTo('${taskTitle}')" ondragover="allowDrop(event)" class="task-body" id="task-body${i}">
                    <div class="task-body-flex">
                        <span>${taskName}</span>
                        <img  onclick="openAddTask()" id="task-img${i}" src="./assets/img/board_plus.png">
                    </div>
